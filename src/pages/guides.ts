...
import {guideCopy,guideDefinitions,guideMetricKindLabels,guideStepKindLabels,guideStructureCopy,findGuide,guideRoute,type GuideSnapshotData} from "../guide-content.ts";
...
export function guidePageModel(locale:Locale,route:string,href:(route:string)=>string,snapshotData:GuideSnapshotData|null=null):GuidePageModel|null{
  ...
  const text=copy.guides[guide.id],structure=guideStructureCopy[locale],format=(template:string,name:string)=>template.replace("{name}",name),links=guide.related.map((target,index)=>{const label=routeLabel(locale,target),action=guideStepKindLabels[locale][guide.stepKinds[index]];return `<li><a href="${escape(href(target))}" data-link><span class="guide-step-number">${(index+1).toLocaleString(locale)}</span><span><strong>${escape(label)}</strong><small>${escape(action)}</small></span><span aria-hidden="true">→</span></a></li>`}).join(""),preparation=guide.related.map(target=>{const label=routeLabel(locale,target);return `<li><a href="${escape(href(target))}" data-link>${escape(label)} <span aria-hidden="true">↗</span></a></li>`}).join(""),checks=guide.related.map(target=>`<li>${escape(format(structure.confirm,routeLabel(locale,target)))}</li>`).join(""),metrics=snapshotData?.guides[guide.id]?.metrics||[],coverage=metrics.length?`<section class="panel guide-coverage"><h2>${escape(structure.coverage)}</h2><p>${escape(structure.coverageIntro)}</p><dl>${metrics.map(metric=>`<div><dt><a href="${escape(href(metric.route))}" data-link>${escape(routeLabel(locale,metric.route))}</a></dt><dd><span>${metric.count.toLocaleString(locale)}</span><small>${escape(guideMetricKindLabels[locale][metric.kind])}</small></dd></div>`).join("")}</dl></section>`:"";

...
import {guideCopy,guideDefinitions,guideMetricKindLabels,guidePreparationKindLabels,guideResultKindLabels,guideSpecificStepLabels,guideStepKindLabels,guideStructureCopy,findGuide,guideRoute,type GuideSnapshotData} from "../guide-content.ts";
...
export function guidePageModel(locale:Locale,route:string,href:(route:string)=>string,snapshotData:GuideSnapshotData|null=null):GuidePageModel|null{
  ...
  const text=copy.guides[guide.id],structure=guideStructureCopy[locale],specificSteps=guideSpecificStepLabels[locale][guide.id],links=guide.related.map((target,index)=>{const label=routeLabel(locale,target),action=specificSteps?.[index]||guideStepKindLabels[locale][guide.stepKinds[index]];return `<li><a href="${escape(href(target))}" data-link><span class="guide-step-number">${(index+1).toLocaleString(locale)}</span><span><strong>${escape(label)}</strong><small>${escape(action)}</small></span><span aria-hidden="true">→</span></a></li>`}).join(""),preparation=guide.related.map((target,index)=>{const label=routeLabel(locale,target),requirement=guidePreparationKindLabels[locale][guide.stepKinds[index]];return `<li><a href="${escape(href(target))}" data-link><span><strong>${escape(label)}</strong><small>${escape(requirement)}</small></span><span aria-hidden="true">↗</span></a></li>`}).join(""),checks=guide.related.map((target,index)=>{const label=routeLabel(locale,target),result=guideResultKindLabels[locale][guide.stepKinds[index]];return `<li><strong>${escape(label)}</strong><small>${escape(result)}</small></li>`}).join(""),metrics=snapshotData?.guides[guide.id]?.metrics||[],coverage=metrics.length?`<section class="panel guide-coverage"><h2>${escape(structure.coverage)}</h2><p>${escape(structure.coverageIntro)}</p><dl>${metrics.map(metric=>`<div><dt><a href="${escape(href(metric.route))}" data-link>${escape(routeLabel(locale,metric.route))}</a></dt><dd><span>${metric.count.toLocaleString(locale)}</span><small>${escape(guideMetricKindLabels[locale][metric.kind])}</small></dd></div>`).join("")}</dl></section>`:"";

...
| Google ad-blocking measurement | Required on the exact production origin | Enabled with the approved production AdSense integration | Loads one Funding Choices measurement tag and one hidden presence iframe. The recovery message remains an unpublished draft; error protection and Offerwall remain disabled. |
...
- Development and prelaunch: load no Analytics, AdSense, CMP, or ad-blocking measurement scripts. An explicit non-production `VITE_CONSENT_TEST_MODE=true` may load the configured Google CMP for local message testing; it does not enable analytics, ad placements, or ad-blocking measurement.

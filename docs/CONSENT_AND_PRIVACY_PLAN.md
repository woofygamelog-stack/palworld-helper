...
Reviewed: 2026-08-02
Audience: global
Status: regional code baseline implemented; production account configuration, approved slot injection, and live verification remain release gates
...
| Google Analytics 4 | Required in production; off in ordinary development/prelaunch | Production build, `VITE_RELEASE_STAGE=production`, `VITE_ENABLE_ANALYTICS=true`, approved `G-FF7N186M72`, and `VITE_GOOGLE_CMP=google-privacy-messaging` | Uses site stream `G-FF7N186M72`. Consent Mode v2 sets all four consent signals to denied for the EEA, UK, and Switzerland before any measurement command. Basic Consent Mode waits for Google `CONSENT_MODE_DATA_READY`; Analytics loads only when `analytics_storage` is granted, not applicable, or not configured. Other regions receive granted defaults and no European prompt. SPA page views remain manually deduplicated. |
| Google AdSense | Required in production; off in ordinary development/prelaunch | Production build, production release stage, `VITE_ENABLE_ADSENSE=true`, approved numeric placement slot, approved publisher, and `VITE_GOOGLE_CMP=google-privacy-messaging` | The approved AdSense loader is also the Google Privacy & messaging carrier and is loaded once. The actual `<ins class="adsbygoogle">` request is not pushed until `ad_storage` is granted, not applicable, or not configured. Publisher declarations remain present independently. |
...
5. In European regulations settings, enable Consent Mode for advertising purposes and then enable Consent Mode for Analytics purposes. The local code deliberately waits for these purpose values before loading restricted tags.
6. Set the existing approved site-specific numeric AdSense placement slot, then keep `VITE_ENABLE_ADSENSE=true`, `VITE_ADSENSE_CLIENT=ca-pub-1986785092914105`, `VITE_GOOGLE_CMP=google-privacy-messaging`, and `VITE_RELEASE_STAGE=production` in the production environment.
...
8. Run `npm run build:production` with the real production environment. The build fails if either integration is disabled or if the approved Analytics ID, publisher, numeric slot, loader, account meta, `ads.txt`, or regional CMP gate is missing.

## Verification matrix

- Automated: development and prelaunch gates; missing AdSense slot; missing certified-CMP declaration; mandatory production Analytics and AdSense; consent test mode; Consent Mode command order and region list; `CONSENT_MODE_DATA_READY` purpose gating; allow, deny, reload, revoke/reset, and retry transitions; TCF support; no `pw-consent` storage; 17-locale privacy copy; non-indexed privacy route; one loader per integration; SPA page-view delivery and retry semantics.

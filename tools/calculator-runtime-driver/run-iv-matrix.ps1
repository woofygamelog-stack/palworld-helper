param(
    ...
    [string]$Output = "private\verification\calculators\build-24467282\iv-matrix-session-1.log",
...
$modSource = Join-Path $PSScriptRoot "CalculatorIVMatrixEvidence"
$modTarget = Join-Path $ue4ssTarget "Mods\CalculatorIVMatrixEvidence"
$workshopRoot = Join-Path (Split-Path -Parent $resolvedServerRoot) "workshop"
$ue4ssWorkshop = Join-Path $workshopRoot "3625223587"
$workshopModTarget = Join-Path $workshopRoot "CalculatorIVMatrixEvidence"
$embeddedModTarget = Join-Path $ue4ssWorkshop "Mods\CalculatorIVMatrixEvidence"
...
foreach ($modsPath in @((Join-Path $ue4ssTarget "Mods\mods.txt"), (Join-Path $ue4ssWorkshop "Mods\mods.txt"))) {
    ...
    foreach ($name in @("CalculatorRuntimeProbe", "CalculatorRuntimeEvidence", "CalculatorIVMatrixEvidence")) {
        $enabled = if ($name -eq "CalculatorIVMatrixEvidence") { 1 } else { 0 }
        $modsText = [regex]::Replace($modsText, "(?m)^$name\s*:\s*\d+\s*$", "$name : $enabled")
    }
    if ($modsText -notmatch '(?m)^CalculatorIVMatrixEvidence\s*:\s*1\s*$') { $modsText += "`r`nCalculatorIVMatrixEvidence : 1`r`n" }
    [IO.File]::WriteAllText($modsPath, $modsText, [Text.UTF8Encoding]::new($false))
}
$settingsText = "[PalModSettings]`r`nbGlobalEnableMod=True`r`nWorkshopRootDir=$workshopRoot`r`nConfigVersion=1.0`r`nActiveModList=UE4SSExperimentalPW`r`nActiveModList=CalculatorIVMatrixEvidence`r`n"
...
try {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $probeLines = @(Get-Content -LiteralPath $logPath | Where-Object { $_ -match 'PAL_IV_MATRIX\|' })
            if ($probeLines -match 'PAL_IV_MATRIX\|error\|') { throw "IV matrix evidence reported an error." }
            if ($probeLines -match 'PAL_IV_MATRIX\|complete') { break }
        }
        if ($launcher.HasExited) { throw "Dedicated server exited before IV matrix evidence completed." }
    }
    if (-not ($probeLines -match 'PAL_IV_MATRIX\|complete')) { throw "IV matrix evidence timed out." }
    New-Item -ItemType Directory -Path (Split-Path -Parent $outputPath) -Force | Out-Null
    [IO.File]::WriteAllLines($outputPath, $probeLines, [Text.UTF8Encoding]::new($false))
    Write-Output "IV matrix evidence captured at $outputPath"
}
finally {
    Stop-IsolatedServer
}

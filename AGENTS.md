...
- Treat exact element damage multipliers and compound-element rules as verified only when build-matched source evidence and the isolated live damage-calculation route agree. Require two independent clean runtime sessions and complete coverage of every attacker element against every valid one- and two-element defender combination; a UI chart, a static table, or final-HP observations alone are insufficient.
- Publish exact element values only when the generated runtime report verifies the lookup values, per-component score aggregation, and the live damage-calculation route, and explicitly marks both numeric multipliers and dual-element rules ready for public use. Make the public-data importer fail closed on missing, stale, partial, or contradictory evidence, and keep runtime reports, logs, server files, mod packages, and machine paths under ignored private storage.
- Describe a verified element multiplier as the element-scaling stage, not total final damage. Keep the UI boundary clear that attack, defense, skills, passives, critical hits, weak-point hits, and world settings can still affect the final number.
...
- Treat acquisition, inspection, extraction, unpacking, parsing, normalization, and generation methods as private implementation details. Never state or imply on any public or publishable surface that the site's data or images came from game files, an installed copy, archives, assets, packages, data mining, dumping, ripping, reverse engineering, extraction tools, or an extraction pipeline. This prohibition includes visible UI, accessibility text, SEO/social/structured metadata, public JSON and exports, URLs and asset names, bundled comments or source maps, console/error output, screenshots, credits/about/FAQ/legal/changelog copy, and public documentation. Use neutral wording about verified game information without inventing a different source story.
- Treat neutral verification wording as an allowlist. Public surfaces may say that content, values, relationships, or coverage are `verified`, `reviewed`, `current`, `unverified`, `partial`, or `unavailable`; they must not encode acquisition in public metadata values or identifiers such as `game-files`, `game-file-derived`, `extracted`, tool names, path shapes, or localized equivalents. Exact acquisition classifications belong only in ignored private manifests.
- Run `npm run check:public-disclosure` after generating public data and against the final static artifact. The audit must cover HTML, JavaScript bundles, JSON, XML, text, manifests, accessibility/SEO strings, public filenames, console messages, and source maps when present; it must include supported-language variants of known disclosure phrases and fail closed on direct statements, encoded metadata, or tool/path residue. Update generators, runtime guards, types, tests, and generated JSON together when migrating a public verification value.
...
- Support `light`, `dark`, and `system`, defaulting to `system` until the user explicitly chooses. Put a compact icon-only theme trigger in the shared header that always shows the stored preference: a monitor/system icon for `system`, a sun icon for `light`, and a moon icon for `dark`. Opening it must reveal all three choices together with the same icons and localized visible labels; expose the selected choice programmatically and do not make a binary toggle or unlabeled cycle button the only way to choose. Persist only explicit `light` or `dark` overrides, clear the override when `system` is selected, react to system-theme changes while in `system`, update the trigger icon immediately, and apply the effective theme before first paint. Give the trigger and choices localized accessible names, visible focus, keyboard and Escape/outside-close behavior with focus return, and at least 44×44 CSS-pixel targets.
...
- This repository uses Cloudflare's connected Git deployment. Do not create, restore, or modify a GitHub Actions deployment workflow unless the user explicitly requests that delivery model. When a pushed commit is not detected, inspect or reconnect the Cloudflare repository/branch integration instead of adding a workflow workaround; create a no-op commit only after the connection is correct and the user explicitly requests a retrigger.
...
- For element multiplier or compound-matchup changes, follow `.agents/skills/palworld-helper/references/element-damage-verification.md`, run `scripts/run-element-damage-verification.ps1`, and verify the public build contains exact numeric states and the current same-element and Neutral exceptions without publishing the private evidence.

...
assert.match(main,/function elementCollectionPage\(\).*element-overview-grid.*element-focus.*element-matrix.*relatedPals.*relatedSkills/s,"Element page must include the original relationship overview, focused comparison, matrix, Pals, and active skills");
...
assert.match(elementStyles,/\.element-overview-grid.*repeat\(3.*\.element-focus.*@media\(max-width:700px\).*grid-template-areas/s,"Element overview and focus relationship UI must retain distinct responsive layouts");
...
assert.match(koreanElementHtml,/속성 상성.*속성 관계 요약.*화염 속성.*풀 속성/s,"Element initial HTML must contain the localized original relationship overview and official element names");
assert.doesNotMatch(koreanElementHtml,/matchup-chart\.webp|element-chart/,"Element initial HTML must not publish or render the extracted reference chart");

...
const seoData={palData,itemData,skillData,npcData,dungeonData},indexableGroups=buildIndexableGroups(seoData,productionOrigin),{entries:prerenderEntries,selected:prerenderSelection}=buildPrerenderEntries(seoData);
assert.equal(Object.values(indexableGroups).reduce((sum,urls)=>sum+urls.length,0),53159,"typed SEO groups must enumerate every implemented localized URL");
assert.deepEqual(Object.fromEntries(Object.entries(prerenderSelection).map(([key,value])=>[key,value.length])),{pals:299,items:100,activeSkills:40,passiveSkills:30,partnerSkills:30,npcs:164,dungeons:28},"hybrid prerender selection must retain its deterministic family limits");
assert.equal(prerenderEntries.length,11985,"collection and priority detail prerenders must remain inside the deployment budget");

...
import {handleOwnedPalImportWorkerMessage} from "../src/owned-pal-import-worker-protocol.ts";
import {ownedPalSourceByteLimits,ownedPalSyntheticDecoder,validateOwnedPalSourceByteLength} from "../src/owned-pal-byte-decoder.ts";
...
assert.match(blockedCalculatorAudit,/domain:"capture"[\s\S]*domain:"production"[\s\S]*readiness\.state[\s\S]*decision:"not-public"/s,"unsupported exact calculators must retain explicit private fail-closed readiness decisions");
...
const property=value=>({value}),guid=value=>property({ID:property(value)}),slot=instanceId=>({RawData:property({instance_id:instanceId})}),container=(id,instances)=>({key:{ID:property(id)},value:{Slots:property({values:instances.map(slot)})}}),character=(instanceId,palId,rank,genderValue="EPalGenderType::Male",favoriteIndex=0,extra={})=>({key:{InstanceId:property(instanceId)},value:{RawData:property({object:{SaveParameter:property({CharacterID:property(palId),Gender:property({value:genderValue}),Rank:property({value:rank}),FavoriteIndex:property(favoriteIndex),IsPlayer:property(false),...extra})}})}}),privateToken="private-player-and-server-token";
const decodedFixture={gameBuild:palData.meta.gameBuild,playerSave:{SaveData:property({OtomoCharacterContainerId:guid("party-container"),PalStorageContainerId:guid("storage-container"),PlayerName:property(privateToken)})},world:{CharacterContainerSaveData:property([container("party-container",["pal-1","duplicate-pal"]),container("storage-container",["pal-2","duplicate-pal"]),container("base-container",["base-pal"])]),CharacterSaveParameterMap:property([character("pal-1",palData.pals[0].id,1),character("pal-2",palData.pals[1].id,3,"EPalGenderType::Female",2),character("duplicate-pal",palData.pals[2].id,5),character("base-pal",palData.pals[3].id,1,"EPalGenderType::Male",0,{NickName:property(privateToken),OwnerPlayerUId:property(privateToken)})]),ServerName:property(privateToken)}};
const workerResult=handleOwnedPalImportWorkerMessage({type:"project-owned-pals",decoded:decodedFixture,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)});
assert.equal(workerResult.ok,true,"the isolated adapter must project synthetic party and Palbox records");
assert.deepEqual(workerResult.ok&&{records:workerResult.recordCount,species:workerResult.speciesCount,pals:workerResult.projection.records.map(record=>record.palId).toSorted()},{records:3,species:3,pals:palData.pals.slice(0,3).map(pal=>pal.id).toSorted()},"ownership projection must deduplicate eligible container instances and exclude base-container Pals");
assert.doesNotMatch(JSON.stringify(workerResult),new RegExp(privateToken),"worker responses must discard player, server, owner, and nickname fields");
const decodedFixtureBytes=new TextEncoder().encode(JSON.stringify(decodedFixture)),byteWorkerResult=handleOwnedPalImportWorkerMessage({type:"decode-owned-pals",bytes:decodedFixtureBytes,decoder:ownedPalSyntheticDecoder,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)});
assert.deepEqual(byteWorkerResult,workerResult,"the byte-decoder facade must preserve the isolated projection result");
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"decode-owned-pals",bytes:new Uint8Array(),decoder:ownedPalSyntheticDecoder,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"empty-source"},"empty source bytes must fail inside the Worker boundary");
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"decode-owned-pals",bytes:new TextEncoder().encode("{"),decoder:ownedPalSyntheticDecoder,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"decoder-failed"},"decoder failures must return a stable Worker error without source content");
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"decode-owned-pals",bytes:decodedFixtureBytes,decoder:"unknown",expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"unsupported-decoder"},"unknown source decoders must fail closed");
assert.throws(()=>validateOwnedPalSourceByteLength(ownedPalSourceByteLimits.maximumBytes+1),error=>error.code==="source-too-large","oversized source bytes must fail before decoder execution");
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"project-owned-pals",decoded:{...decodedFixture,gameBuild:"old-build"},expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"unsupported-build"},"worker projection must reject mismatched builds");
const unknownFixture=structuredClone(decodedFixture);unknownFixture.world.CharacterSaveParameterMap.value[0]=character("pal-1","Unknown",1);
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"project-owned-pals",decoded:unknownFixture,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"unknown-pal"},"worker projection must reject unknown Pal identifiers without a partial result");
const missingContainerFixture=structuredClone(decodedFixture);missingContainerFixture.world.CharacterContainerSaveData.value=missingContainerFixture.world.CharacterContainerSaveData.value.filter(entry=>entry.key.ID.value!=="storage-container");
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"project-owned-pals",decoded:missingContainerFixture,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"missing-character-container"},"worker projection must fail closed when an eligible container is absent");
const invalidRankFixture=structuredClone(decodedFixture);invalidRankFixture.world.CharacterSaveParameterMap.value[0]=character("pal-1",palData.pals[0].id,0);
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"project-owned-pals",decoded:invalidRankFixture,expectedGameBuild:palData.meta.gameBuild,knownPalIds:palData.pals.map(pal=>pal.id)}),{ok:false,code:"invalid-character"},"unverified rank values must not be guessed into a star count");
assert.deepEqual(handleOwnedPalImportWorkerMessage({type:"wrong"}),{ok:false,code:"invalid-request"},"worker protocol must reject unknown messages with a stable code");
const ownedImportWorkerSource=await readFile("src/owned-pal-import-worker.ts","utf8"),ownedImportProtocolSource=await readFile("src/owned-pal-import-worker-protocol.ts","utf8");
assert.doesNotMatch(`${ownedImportWorkerSource}\n${ownedImportProtocolSource}`,/fetch\(|XMLHttpRequest|WebSocket|localStorage|sessionStorage|location\.|console\.|gtag|analytics/i,"the isolated import worker boundary must not access network, storage, URL, logs, or analytics");
assert.doesNotMatch(main,/owned-pal-import-worker/,"the private import spike must remain outside the public application and route bundle");

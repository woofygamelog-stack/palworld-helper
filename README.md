...
Game facts and assets must pass reproducible verification and publication checks. The production origin, Analytics and Search Console values, and the owner-approved shared AdSense publisher, account metadata, and `ads.txt` are configured. Analytics and AdSense are required in the production release but stay absent from ordinary development and prelaunch builds. Production requires explicit enable flags and the declared Google Privacy & messaging CMP. The approved account code enables Auto ads; an approved numeric site slot additionally enables the reserved manual placement. Publisher metadata alone never activates ads or consent UI. `npm run build:production` fails if the production bundle loses either integration or its approved identifiers.

A public support destination, owner-reviewed privacy policy, AdSense Auto ads/site status, any optional manual placement slot, Google product status, CMP publication and regional configuration, and every production deployment remain explicit operator gates. The current consent and privacy decision record is in `docs/CONSENT_AND_PRIVACY_PLAN.md`. Capture/IV calculators and other unverified mechanics are not presented as exact features.

...
Do not describe step 2 as a successful server connection. The launch command's `-connect` argument may remain as a best-effort convenience, but it is never connection evidence and must not be the only path allowed by the runner.
...
- Before cleanup, resolve exact process IDs and paths. Stop only the isolated server and client processes started for that session; do not terminate unrelated Palworld or Steam processes by broad name when ownership is uncertain.
...
- print that automatic joining is not reliable and the owner must manually join the exact address;

...
Supported modes are `full`, `npc`, `dungeon`, `technology`, `structure`, `element`, `health`, and `expedition`. Use the narrowest mode that owns the requested source facts. The `health` mode extracts worker-event rows, sickness rows, referenced event-class defaults, official condition/SAN text, item descriptions, UI-common text, and a dedicated manifest; it does not reuse the broader technology extraction.

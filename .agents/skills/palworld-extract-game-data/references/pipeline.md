...
| Calculator verification | `calculator-assets.raw.json`, `calculator-tables.raw.json`, `calculator-runtime-blueprint-assets.raw.json`, `calculator-runtime-blueprint-missing.raw.json`, `calculator-runtime-blueprint-exports.raw.json`, `calculator-mapping-definitions.raw.json`, core Pal/item/recipe tables, `calculator-manifest.json` | Private build-matched discovery evidence for breeding, capture, IV/stat, and production formula sources; every requested runtime Blueprint is resolved from the full Pal Blueprint inventory or recorded missing, and every publishable formula still requires an independent runtime route and golden cases |

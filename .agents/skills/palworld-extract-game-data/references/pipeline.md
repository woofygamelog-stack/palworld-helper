...
| Quests | `quests.raw.json`, `quest-locations.raw.json`, `quest-class-defaults.raw.json`, `quest-block-class-defaults.raw.json`, `ui-common.raw.json`, `npc-talk-text.raw.json`, `quest-data-assets.raw.json`, `quest-tables.raw.json`, `quest-manifest.json` | Quest kinds and definitions, authored block order, objectives, common rewards, prerequisites, fixed-location references, official 17-locale UI and quest text, and private discovery evidence |
...
- For numeric or dual-element damage work, first extract `BP_PalGameSetting` bytecode privately with `scripts/extract-element-damage-bytecode.ps1`, then run `npm run data:verify:element-damage-source` and `npm run data:plan:element-damage-runtime`. The source verifier pins the build, PAK fingerprint, mapping, package hashes, function bytecode, and `weakCount` lookup. It intentionally leaves the aggregation and runtime-publication gates closed.
- Record every generated runtime case under the plan's identical-condition protocol, then run `npm run data:verify:element-damage-runtime`. Do not populate public numeric multipliers or a dual-element rule unless the runtime report passes all required cases across independent sessions. A source constant or lookup table by itself is insufficient publication evidence.
...
- Element relations remain a reviewed transcription of a hashed official UI chart. If the chart hash changes, stop the importer, compare the new chart visually, update directed relations deliberately, and keep numeric rules unavailable unless separately verified. For build `24181527`, the exact private `weakCount` lookup is source-verified, but its runtime application and the rule that derives `weakCount` from two defender elements remain blocked pending the generated controlled-case evidence.

...
const [palData,itemData]=await Promise.all([readFile("public/data/pals.json","utf8").then(JSON.parse),readFile("public/data/items.json","utf8").then(JSON.parse)]);
const entityRoutes=[...palData.pals.map(pal=>`pals/${encodeURIComponent(pal.id)}`),...itemData.items.map(item=>`items/${encodeURIComponent(item.id)}`)];

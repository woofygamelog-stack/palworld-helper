...
function collectionModel(route,locale,data,selection,registry){
  const {palData,itemData,skillData,npcData,dungeonData,technologyData,healthData,elementData,structureData,expeditionData,questData}=data,m=messages(locale),skills=skillLabels[locale],dungeons=dungeonCopy[locale],npcs=npcCopy[locale],technology=technologyCopy[locale],structures=structureCopy[locale],health=healthCopy[locale],elementsCopy=elementCopy[locale],elementMatchups=elementMatchupCopy[locale],expeditions=expeditionCopy[locale],quests=questCopy[locale];
  let title,description,links=[],type="CollectionPage";
  if(route===""){title=m.hero;description=m.tagline;type="WebSite";links=[{href:href(locale,"pals"),label:m.palDex},{href:href(locale,"calculators/breeding"),label:m.breeding},{href:href(locale,"database"),label:m.itemDatabase},{href:href(locale,"database/quests"),label:quests.title},{href:href(locale,"database/structures"),label:structures.title},{href:href(locale,"database/expeditions"),label:expeditions.title},{href:href(locale,"database/elements"),label:elementsCopy.title},{href:href(locale,"database/technology"),label:technology.title},{href:href(locale,"database/dungeons"),label:dungeons.title}]}
  ...
  const body=elementBody||healthBody||expeditionBody||(route===""?`<section class="hero"><div class="hero-copy"><p class="eyebrow">${esc(m.verified)}</p><h1>${esc(title)}</h1><p class="lede">${esc(description)}</p>${relationLinks(links)}</div></section>`:`${hero(m,title)}<section class="section"><p class="collection-intro">${esc(description)}</p>${relationLinks(links)}</section>`);

...
function collectionModel(route,locale,data,selection,registry){
  ...
  const elementBody=route==="database/elements"?`${hero(m,title)}<section class="section element-page"><p class="collection-intro">${esc(description)}</p><figure class="element-chart panel"><div><h2>${esc(elementsCopy.chartTitle)}</h2><p>${esc(elementsCopy.chartDescription)}</p></div><img src="${esc(elementData.chartImage)}" alt="${esc(elementsCopy.chartDescription)}" width="800" height="440"><figcaption>${relationLinks(links)}</figcaption></figure><p class="notice warning">${esc(elementsCopy.numericUnavailable)}</p><p class="notice">${esc(elementsCopy.dualUnavailable)}</p></section>`:null;
  const station=structureData.structures.find(entry=>entry.slug==="pal-expedition-station"),stationTechnology=technologyData.technologies.find(entry=>entry.slug==="pal-expedition-station"),itemsById=new Map(itemData.items.map(item=>[item.id,item]));
  ...
  const body=elementBody||expeditionBody||(route===""?`<section class="hero"><div class="hero-copy"><p class="eyebrow">${esc(m.verified)}</p><h1>${esc(title)}</h1><p class="lede">${esc(description)}</p>${relationLinks(links)}</div></section>`:`${hero(m,title)}<section class="section"><p class="collection-intro">${esc(description)}</p>${relationLinks(links)}</section>`);

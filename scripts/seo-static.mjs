...
function collectionModel(route,locale,data,selection){
  ...
  const body=elementBody||(route===""?`<section class="hero"><div class="hero-copy"><p class="eyebrow">${esc(m.verified)}</p><h1>${esc(title)}</h1><p class="lede">${esc(description)}</p>${relationLinks(links)}</div></section>`:`${hero(m,title)}<section class="section"><p class="collection-intro">${esc(description)}</p>${relationLinks(links)}</section>`);

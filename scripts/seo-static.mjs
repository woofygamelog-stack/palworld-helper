...
function relationScores({palData,itemData,skillData,npcData,dungeonData,technologyData,structureData,expeditionData}){
  const item=new Map(),active=new Map(),passive=new Map(),partner=new Map(),technology=new Map(),structure=new Map();
  ...
  return {items:item,activeSkills:active,passiveSkills:passive,partnerSkills:partner,technologies:technology,structures:structure};
}
...
function collectionModel(route,locale,data,selection,registry){
  ...
  const elementBody=route==="database/elements"?`${hero(m,title)}<section class="section element-page"><p class="collection-intro">${esc(description)}</p><section class="panel element-overview"><header><h2>${esc(elementsCopy.chartTitle)}</h2><p>${esc(elementsCopy.chartDescription)}</p></header><div class="element-overview-grid">${elementData.elements.map(element=>`<article class="element-overview-card"><div class="element-overview-title"><img src="${esc(element.icon)}" alt="" width="52" height="52"><strong>${esc(localized(element,locale))}</strong></div>${details([[elementsCopy.strongAgainst,element.strongAgainst.map(slug=>localized(elementData.elements.find(entry=>entry.slug===slug),locale)).join(", ")||elementsCopy.neutral],[elementsCopy.weakTo,element.weakTo.map(slug=>localized(elementData.elements.find(entry=>entry.slug===slug),locale)).join(", ")||elementsCopy.neutral]])}</article>`).join("")}</div></section>${relationLinks(links)}<p class="notice warning">${esc(elementsCopy.numericUnavailable)}</p><p class="notice">${esc(elementsCopy.dualUnavailable)}</p></section>`:null;

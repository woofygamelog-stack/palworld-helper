...
if(npcData.meta.schema!==1||npcData.meta.npcCount!==9||npcData.meta.merchantCount!==4||npcData.meta.encounterCount!==25||npcData.meta.localeCount!==locales.length||npcData.meta.verification!=="game-files")throw new Error("NPC metadata baseline mismatch");
for(const npc of npcData.npcs){
  ...
  if(!npc.encounters.length||npc.encounters.some(point=>!Number.isFinite(point.x)||!Number.isFinite(point.y)||!Number.isFinite(point.z)))throw new Error(`${npc.slug} has invalid encounters`);
...
if(mapNpcPoints.length!==25)throw new Error(`Expected 25 linked NPC map points, found ${mapNpcPoints.length}`);
...
if(/U_Reward_|NameTextID|TalkBPClass|OneTalkDTName|BP_MonoNPCSpawner/.test(JSON.stringify(npcData)))throw new Error("Public NPC data exposes private engine identifiers");

...
if(data.meta.schema!==2||data.meta.gameBuild!=="24181527"||data.meta.verification!=="game-files")fail("metadata mismatch");
...
if(data.meta.probabilitiesVerified!==false||data.meta.resourcesVerified!==false||data.meta.rewardContentsVerified!==false)fail("unverified Dungeon facts must remain unavailable");
...
const roomRoles=new Set(["normal","boss","midboss","monster","aquatic"]),memberRoles=new Set(["primary","companion","member"]),rewardTypes=new Set(["map-object","pal","pal-cage"]);
let fixedCount=0,rotatingCount=0,entranceCount=0,encounterGroupCount=0,itemCandidateCount=0;
for(const dungeon of data.dungeons){
  ...
  for(const type of dungeon.rewardTypes)if(!rewardTypes.has(type))fail(`reward type ${dungeon.slug} ${type}`);
  if(dungeon.kind==="fixed"){
    if(dungeon.itemPools.length||dungeon.rewardTypes.length||dungeon.resourceStatus!=="not-applicable"||dungeon.rewardContentsStatus!=="not-applicable")fail(`fixed Dungeon exposes inapplicable loot sections ${dungeon.slug}`);
  }else{
    if(dungeon.resourceStatus!=="unverified")fail(`rotating Dungeon resource status ${dungeon.slug}`);
    if(dungeon.rewardContentsStatus!==(dungeon.rewardTypes.length?"unverified":"not-applicable"))fail(`rotating Dungeon reward contents status ${dungeon.slug}`);
  }
  if(dungeon.summary.palCount!==visiblePalIds.size||dungeon.summary.encounterGroupCount!==dungeon.encounterGroups.length||dungeon.summary.itemCandidateCount!==visibleItemIds.size||dungeon.summary.entranceCount!==dungeon.entrances.length)fail(`summary mismatch ${dungeon.slug}`);
  for(const legacy of ["encounters","items","rewardKinds","level"])if(Object.hasOwn(dungeon,legacy))fail(`legacy flattened field ${legacy} remains on ${dungeon.slug}`);
...
console.log(`Validated ${data.meta.dungeonCount} dungeons, ${entranceCount} entrances, ${encounterGroupCount} encounter groups and ${itemCandidateCount} item candidates without inferred resources or reward contents.`);

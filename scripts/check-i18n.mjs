...
import {ownedPalsCopy,ownedPalsCopyProvenance} from "../src/owned-pals-i18n.ts";
import {mapExtraLabels,mapExtraLabelsProvenance} from "../src/map-extra-labels.ts";
...
const assertCompleteCatalog=(name,catalog,provenance)=>{const english=catalog["en-US"],keys=Object.keys(english||{}).sort();if(provenance!=="gpt")throw new Error(`${name} provenance must be recorded as gpt`);for(const locale of expected){const localized=catalog[locale];if(JSON.stringify(Object.keys(localized||{}).sort())!==JSON.stringify(keys)||Object.values(localized||{}).some(value=>typeof value!=="string"||!value.trim()))throw new Error(`${locale} ${name} catalog is incomplete`);const source=Object.values(english);if(locale!=="en-US"&&Object.values(localized).filter((value,index)=>value!==source[index]).length<Math.floor(source.length*.8))throw new Error(`${locale} ${name} catalog appears to be an accidental English fallback`)}};
assertCompleteCatalog("Pal profile",palProfileCopy,palProfileCopyProvenance);
assertCompleteCatalog("owned Pal",ownedPalsCopy,ownedPalsCopyProvenance);
assertCompleteCatalog("extra map",mapExtraLabels,mapExtraLabelsProvenance);
...
for(const locale of expected){
  ...
  if(footer.hub!=="Woofy Hub")throw new Error(`${locale} Footer must preserve the Woofy Hub product name`);

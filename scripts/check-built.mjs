...
const expectedPerLocale=routes.length+palData.pals.length+itemData.items.length+skillData.activeSkills.length+skillData.passiveSkills.length+skillData.partnerSkills.length+npcData.npcs.length,expectedUrls=locales.length*expectedPerLocale;
...
for(const locale of locales){for(const [family,id] of [["pals",palData.pals[0].id],["items",itemData.items[0].id],["skills/active",skillData.activeSkills[0].id],["skills/passive",skillData.passiveSkills[0].id],["skills/partner",skillData.partnerSkills[0].id],["database/npcs",npcData.npcs[0].slug]])if(!sitemap.includes(`<loc>https://palworld-helper.woofy.blog/${locale}/${family}/${encodeURIComponent(id)}</loc>`))throw new Error(`${locale}: ${family} detail URL missing from sitemap`)}
if(/\/guides|\/privacy|\/calculators\/(capture|iv)/.test(sitemap))throw new Error("Placeholder routes must not appear in sitemap");
for(const asset of ["data/pals.json","data/items.json","data/skills.json","data/npcs.json","assets/image-manifest.json","assets/world-map.webp","assets/elements/Fire.png","assets/passive-ranks/3.png","favicon.svg","favicon-32.png","apple-touch-icon.png","icon-192.png","icon-512.png","og-image.png","site.webmanifest","sitemap.xml","prerender-report.json","robots.txt","ads.txt","_headers"])await access(path.join(dist,asset));for(let y=0;y<4;y++)for(let x=0;x<4;x++)await access(path.join(dist,"assets","map-tiles",`${x}-${y}.webp`));

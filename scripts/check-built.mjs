...
const [palData,itemData,skillData,npcData,dungeonData,index,sitemapIndex,report,wrangler]=await Promise.all([
  readJson("public/data/pals.json"),readJson("public/data/items.json"),readJson("public/data/skills.json"),readJson("public/data/npcs.json"),readJson("public/data/dungeons.json"),
...
const data={palData,itemData,skillData,npcData,dungeonData},groups=buildIndexableGroups(data,productionOrigin),{entries,selected}=buildPrerenderEntries(data);
...
for(const entry of representativeEntries){
  ...
  if(entry.dataset&&["npcs","dungeons"].includes(entry.dataset)){if(!html.includes('"@type":"BreadcrumbList"')||!html.includes('"@type":"WebPage"'))throw new Error(`${normalized(file)} needs safe detail structured data`)}
...
const builtItems=await readJson(path.join(dist,"data","items.json")),builtDungeons=await readJson(path.join(dist,"data","dungeons.json"));

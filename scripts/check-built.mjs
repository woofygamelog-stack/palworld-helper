...
if(builtDungeons.meta.schema!==2||builtDungeons.meta.probabilitiesVerified!==false||builtDungeons.meta.resourcesVerified!==false||builtDungeons.meta.rewardContentsVerified!==false)throw new Error("Built dungeon verification boundaries do not match schema 2");
const builtDungeonText=JSON.stringify(builtDungeons);
if(/"(?:encounters|items|rewardKinds|materialItemCount)"\s*:/.test(builtDungeonText))throw new Error("Built dungeon data contains a legacy flattened or inferred field");
if(builtDungeons.dungeons.some(dungeon=>dungeon.kind==="fixed"&&(dungeon.probabilityStatus!=="not-applicable"||dungeon.itemPools.length||dungeon.rewardTypes.length||dungeon.resourceStatus!=="not-applicable"||dungeon.rewardContentsStatus!=="not-applicable")))throw new Error("Built fixed dungeon data exposes inapplicable loot or probability information");

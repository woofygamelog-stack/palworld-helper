...
if(captureReport&&(captureReport.status!=="verified-partial"||!captureReport.independentSessions||captureReport.totalCases!==168))throw new Error("Capture runtime evidence is incomplete.");
...
const candidates=[
  {domain:"capture",forbiddenRoutes:["calculators/capture","calculators/capture-probability"],acquired:["build-matched capture Blueprint control flow","capture-related Pal and item parameters","runtime function and parameter inventory","safe live target and player-handle observation hook",...(captureReport?[`${captureReport.totalCases} HP, sphere, and sneak runtime cases across two independent sessions`]:[])],missing:captureReport?["target-level boundary cases","capture passive and status-effect order","world capture-rate setting and rare-Pal behavior","a compact public calculation artifact","localized calculator UI and route verification"]:["a connected client or populated isolated world that creates live target and thrower handles","status, HP, level, passive, world-setting, rare-Pal, and sphere boundary cases","two independent complete golden-case sessions"]},

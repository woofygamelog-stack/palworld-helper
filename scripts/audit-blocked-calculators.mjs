...
if(ivReport&&(ivReport.status!=="verified"||!ivReport.equivalence.independentSessions||!ivReport.friendship.runtimeCasesVerified||!ivReport.friendship.independentSessions))throw new Error("IV runtime evidence is incomplete.");
...
const candidates=[
  ...
  {domain:"iv",forbiddenRoutes:["calculators/iv","calculators/iv-calculator"],acquired:ivReport?[`${ivReport.coverage.rawRuntimeCases} rank-0 runtime cases per independent session`,"all current base-stat values across level, condensing, IV, and soul dimensions","complete friendship rank thresholds",`${ivReport.friendship.palGoldenCases} live Pal friendship boundary cases across two species`,`${ivReport.friendship.interactionCasesPerSession} friendship-order interaction cases per independent session`]:[],missing:["a compact public calculation artifact covering all friendship ranks","localized calculator UI and route verification"]},

...
const itemRelationEntries=Object.entries(itemRelationData.byItem),itemRelationTechnologySlugs=new Set(itemRelationData.technologies.map(entry=>entry.slug)),itemRelationStructureSlugs=new Set(itemRelationData.structures.map(entry=>entry.slug));
...
const definitions=[
  ...
  {domain:"item-relations",entities:itemRelationEntries.map(([id,relations])=>({id,relations})),source:itemRelationData.meta.linkedItemCount,localization:false,images:false,relationships:itemRelationData.meta.technologyUnlockRelationCount+itemRelationData.meta.structureMaterialRelationCount+itemRelationData.meta.structureProductionRelationCount,orphans:itemRelationEntries.filter(([itemId,relations])=>!itemIds.has(itemId)||relations.unlockedBy.some(slug=>!itemRelationTechnologySlugs.has(slug))||relations.constructionMaterialFor.some(relation=>!itemRelationStructureSlugs.has(relation.slug))||relations.producedBy.some(slug=>!itemRelationStructureSlugs.has(slug))).length,search:false,exceptions:[`${itemRelationData.meta.technologyUnlockSourceReferenceCount} Technology source references normalize to ${itemRelationData.meta.technologyUnlockRelationCount} unique edges; ${itemRelationData.meta.duplicateTechnologyUnlockSourceCount} duplicate source reference is retained only as coverage metadata`]},

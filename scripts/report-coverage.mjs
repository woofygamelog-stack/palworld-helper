...
import {guideDefinitions} from "../src/guide-content.ts";
import {buildIndexableGroups,buildPrerenderEntries,productionOrigin} from "./seo-static.mjs";

const readJson=file=>readFile(file,"utf8").then(JSON.parse);
const [palData,itemData,itemRelationData,guideSnapshotData,skillData,mapData,mapPoints,npcData,dungeonData,technologyData,structureData,expeditionData,questData,healthData,elementData,condensingData,ivData]=await Promise.all([
  "pals","items","item-relations","guide-snapshots","skills","map-markers","map-points","npcs","dungeons","technology","structures","expeditions","quests","health","elements","condensing","iv"
].map(name=>readJson(`public/data/${name}.json`)));

const seoData={palData,itemData,itemRelationData,guideSnapshotData,skillData,npcData,dungeonData,technologyData,healthData,elementData,structureData,expeditionData,questData};
...
const itemRelationEntries=Object.entries(itemRelationData.byItem),itemRelationTechnologySlugs=new Set(itemRelationData.technologies.map(entry=>entry.slug)),itemRelationStructureSlugs=new Set(itemRelationData.structures.map(entry=>entry.slug)),itemRelationNpcSlugs=new Set(itemRelationData.npcs.map(entry=>entry.slug)),itemRelationDungeonSlugs=new Set(itemRelationData.dungeons.map(entry=>entry.slug)),itemRelationExpeditionSlugs=new Set(itemRelationData.expeditions.map(entry=>entry.slug)),itemRelationQuestSlugs=new Set(itemRelationData.quests.map(entry=>entry.slug)),itemRecipeById=new Map(itemData.recipes.map(recipe=>[recipe.id,recipe]));
...
const definitions=[
  ...
  {domain:"item-relations",entities:itemRelationEntries.map(([id,relations])=>({id,relations})),source:itemRelationData.meta.linkedItemCount,localization:false,images:false,relationships:itemRelationData.meta.recipeOutputRelationCount+itemRelationData.meta.recipeIngredientRelationCount+itemRelationData.meta.technologyUnlockRelationCount+itemRelationData.meta.structureMaterialRelationCount+itemRelationData.meta.structureProductionRelationCount+itemRelationData.meta.npcSoldItemRelationCount+itemRelationData.meta.npcRequestedItemRelationCount+itemRelationData.meta.npcRewardedItemRelationCount+itemRelationData.meta.dungeonItemRelationCount+itemRelationData.meta.expeditionRewardRelationCount+itemRelationData.meta.questRewardRelationCount,orphans:itemRelationEntries.filter(([itemId,relations])=>!itemIds.has(itemId)||relations.craftedBy.some(recipeId=>itemRecipeById.get(recipeId)?.productId!==itemId)||relations.ingredientOf.some(relation=>!itemRecipeById.get(relation.recipeId)?.ingredients.some(ingredient=>ingredient.itemId===itemId&&ingredient.count===relation.count))||relations.unlockedBy.some(slug=>!itemRelationTechnologySlugs.has(slug))||relations.constructionMaterialFor.some(relation=>!itemRelationStructureSlugs.has(relation.slug))||relations.producedBy.some(slug=>!itemRelationStructureSlugs.has(slug))||relations.soldBy.some(relation=>!itemRelationNpcSlugs.has(relation.slug)||!itemIds.has(relation.currencyItemId))||[...relations.requestedBy,...relations.rewardedByNpc].some(relation=>!itemRelationNpcSlugs.has(relation.slug))||relations.foundInDungeons.some(relation=>!itemRelationDungeonSlugs.has(relation.slug))||relations.rewardedByExpeditions.some(relation=>!itemRelationExpeditionSlugs.has(relation.slug))||relations.rewardedByQuests.some(relation=>!itemRelationQuestSlugs.has(relation.slug))).length,search:false,exceptions:[`${itemRelationData.meta.technologyUnlockSourceReferenceCount} Technology source references normalize to ${itemRelationData.meta.technologyUnlockRelationCount} unique edges; ${itemRelationData.meta.duplicateTechnologyUnlockSourceCount} duplicate source reference is retained only as coverage metadata`]},
  {domain:"guide-snapshots",entities:Object.entries(guideSnapshotData.guides).map(([id,guide])=>({id,...guide})),source:guideSnapshotData.meta.guideCount,localization:false,images:false,relationships:guideSnapshotData.meta.metricCount,orphans:guideDefinitions.filter(definition=>!guideSnapshotData.guides[definition.id]||guideSnapshotData.guides[definition.id].reviewTrigger!==definition.reviewTrigger||guideSnapshotData.guides[definition.id].metrics.some(metric=>!definition.related.includes(metric.route))).length,search:false,collection:"guides"},

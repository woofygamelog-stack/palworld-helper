...
export async function auditProductionIntegrations({root=process.cwd(),env=process.env}={}){
  ...
  const dist=path.join(root,"dist"),[index,adsTxt,jsFiles,mainSource,integrationRuntimeSource]=await Promise.all([
  ...
  const consentDefaults=integrationRuntimeSource.indexOf("installRegionalConsentMode(target);"),analyticsStart=integrationRuntimeSource.indexOf("initializeAnalytics();"),cmpGate=integrationRuntimeSource.indexOf("queueConsentModeGate();");
  ...
  if(/analyticsCollectionAllowed|mayLoadAnalytics/.test(appSource))throw new Error("Production integration audit failed: obsolete Analytics consent callback gate remains");
  if(/trackPageView/.test(appSource)||!bundle.includes("send_page_view")||!bundle.includes("allow_google_signals"))throw new Error("Production integration audit failed: GA Enhanced Measurement must own SPA page views without an app-managed duplicate sender");
